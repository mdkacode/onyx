...
def _extract_error_detail(response: httpx.Response) -> str:
    ...
    try:
        ...
        logger.debug("Naarni MCP Server: error body was not JSON (%s)", exc)
...
async def search_indexed_documents(
    ...
    """
    logger.info(
        "Naarni MCP Server: document search: query='%s', sources=%s, document_sets=%s",
        query,
        source_types,
        document_set_names,
    )
    ...
    if not sources:
        ...
        return _error_payload(
            ...
            "through Naarni before calling search_indexed_documents."
        )
    ...
    if source_types is not None:
        ...
        for source_str in source_types:
            try:
                ...
                logger.warning(
                    "Naarni MCP Server: Invalid source type '%s' - skipping",
                    source_str,
                )
    ...
    try:
        ...
        logger.warning(
            "Naarni MCP Server: invalid time_cutoff '%s' (%s); continuing without time filter",
            time_cutoff,
            err,
        )
        parsed_cutoff = None
    ...
    try:
        ...
        logger.info(
            "Naarni MCP Server: Internal search returned %s results", len(results)
        )
        return {"results": results}
    except Exception as err:
        logger.error("Naarni MCP Server: Document search error: %s", err, exc_info=True)
...
async def search_web(
    ...
    ```
    """
    logger.info("Naarni MCP Server: Web search: query='%s', limit=%s", query, limit)
    ...
    try:
        ...
        logger.error("Naarni MCP Server: Web search error: %s", e, exc_info=True)
        return {
...
async def open_urls(
    ...
    ```
    """
    logger.info("Naarni MCP Server: Open URL: fetching %s URLs", len(urls))
    ...
    try:
        ...
        logger.error("Naarni MCP Server: URL fetch error: %s", err, exc_info=True)
