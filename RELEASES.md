...
## 2026-04-22 — PDF download works, fleet dates now in IST
...
- **PDF downloads fixed.** When the assistant generates a PDF, the chat
  ...
  reliable fallback.
- **Fleet date/time is now IST-aware.** When you say "yesterday",
  "last week", or give a specific date, the assistant now interprets
  the date in India time (IST) and queries the Naarni API correctly.
  Previously the tool used UTC for "today", causing off-by-one-day
  errors when users asked questions late at night or early morning.
...
- CI deploy pipeline was failing with a git submodule error
  ...
  `.gitignore` so the mistake can't recur.
